...
public class LevelUIPanelTest {
    ...
    void testUpdateLabel() {
        levelUIPanel.updateLabel(1); // 레벨 1 증가
        assertEquals(2, levelUIPanel.getLevel(), "레벨은 2가 되어야 한다.");

        levelUIPanel.updateLabel(3); // 레벨 3 증가
        assertEquals(5, levelUIPanel.getLevel(), "레벨은 5가 되어야 한다.");
    }
    ...
    void testUpdateLevelData_showsMenu() {
        ...
        // LevelManager로부터 업데이트를 받으면 (showLevelUpMenu(!nowVisible))
        levelUIPanel.updateLevelData(mockGameLevelData);
